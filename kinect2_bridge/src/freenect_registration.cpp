...
void FreenectRegistrationImpl::distort(int mx, int my, float& x, float& y) const
{
...
void FreenectRegistrationImpl::depth_to_color(float mx, float my, float& rx, float& ry) const
{
...
void FreenectRegistration::apply( int dx, int dy, float dz, float& cx, float &cy) const
{
  impl_->apply(dx, dy, dz, cx, cy);
}

void FreenectRegistrationImpl::apply( int dx, int dy, float dz, float& cx, float &cy) const
{
...
void FreenectRegistrationImpl::apply(const libfreenect2::Frame *rgb, const libfreenect2::Frame *depth, libfreenect2::Frame *undistorted, libfreenect2::Frame *registered, const bool enable_filter, libfreenect2::Frame *bigdepth, int *color_depth_map) const
{
  ...
  {
    std::cerr << "One or more input parameter(s) for apply() are invalid, can not continue!" << std::endl;
    return;
  }
  ...
  for (int i = 0; i < size_depth; ++i, ++undistorted_data, ++map_dist, ++map_x, ++map_yi, ++map_c_off)
  {
    if (iter_count % 100000 == 0)
        std::cerr << "Iteration " << i << " of " << size_depth  << " (depth img.)" << std::endl;
    ...
    if (index < 0)
    {
      std::cerr << "distorted depth pixel is outside of the depth image, index: " << index << std::endl;
      *map_c_off = -1;
      *undistorted_data = 0;
      continue;
    }
    ...
    *undistorted_data = z;
    ...
    if(z <= 0.0f){
      std::cerr << "invalid depth value " << z << " , index: " << index << std::endl;
      *map_c_off = -1;
      continue;
    }
    ...
    const int cy = *map_yi;
    // combining offsets
    const long c_off = cx + cy * 1920;

    // std::cerr << "c_off = " << c_off << " <-> " << cx << " + " << cy << " * " << 1920 << std::endl;
    ...
    {
      std::cerr << "c_off outside rgb image: c_off = " << c_off << ", size_color = " << size_color << std::endl;
      *map_c_off = -1;
      continue;
    }
  ...
  if(enable_filter)
  {
    ...
    for(int i = 0; i < size_depth; ++i, ++map_c_off, ++undistorted_data, ++registered_data){
      ...
      if(c_off < 0){
        std::cerr << "c_off outside image (2): " << c_off << std::endl;
        *registered_data = 0;
        continue;
      }
      ...
      {
          current_pixel = 0;
          std::cerr << "current_pixel value: " << current_pixel << " = 0: (" << z << " - " << min_z << ") / " << z << " = " << ((z - min_z) / z) << " > " << filter_tolerance << std::endl;
      }
      else
      {
          current_pixel = *(rgb_data + c_off);
          std::cerr << "current_pixel value: " << current_pixel << std::endl;
      }
      *registered_data = current_pixel;
  }
