...
#include <vector>
#include <algorithm>
#include <kinect2_bridge/freenect_libusb.h>
#include <limits>
#include <cmath>
#include <cstdlib>
#include <cstring>
...
#include <kinect2_bridge/freenect_protocol_command.h>
#include <kinect2_bridge/freenect_protocol_response.h>
...
namespace Freenect
{
    ...
    using namespace Freenect::usb;
    using namespace Freenect::protocol;
    ...
    */
    ...
    public:
        ...
        virtual void setColorFrameListener(Freenect::FrameListener* rgb_frame_listener);
        virtual void setIrAndDepthFrameListener(Freenect::FrameListener* ir_frame_listener);
        ...
        void enumerateDevices()
        {
            ...
            if (num_devices > 0)
            {
                for (int idx = 0; idx < num_devices; ++idx)
                {
                    ...
                    LOG_INFO << "USB device " << idx << ": idVendor = " << dev_desc.idVendor << ", idProduct = " << dev_desc.idProduct;

                    libusb_device_handle* dev_handle;
                    r = libusb_open(dev, &dev_handle);

                    LOG_INFO << "libusb_open returned: " << r << ", LIBUSB_SUCCESS = " << LIBUSB_SUCCESS;
                    ...
                    if (r == LIBUSB_SUCCESS)
                    {
                        ...
                        LOG_INFO << "USB device " << idx << " device descriptor: " << buffer;
    ...
    void Freenect2DeviceImpl::setColorFrameListener(Freenect::FrameListener* rgb_frame_listener)
    {
    ...
    void Freenect2DeviceImpl::setIrAndDepthFrameListener(Freenect::FrameListener* ir_frame_listener)
    {
        // TODO: should only be possible, if not started
    ...
    Freenect2Device* Freenect2Impl::openDevice(int idx, const PacketPipeline* pipeline, bool attempting_reset)
    {
        ...
        if (attempting_reset)
        {
            ...
            if (r == LIBUSB_ERROR_NOT_FOUND)
            {
                ...
                Freenect::this_thread::sleep_for(Freenect::chrono::milliseconds(1000));
    ...
    {
        running_ = true;
        t_ = new Freenect::thread(static_execute, this);
